...
When the native backdrop is active and contrast is white −50% or stronger, the renderer uses a dark-foreground/light-surface palette at every frost level. Every other active combination uses the light-foreground palette. xterm changes theme in place, retains its 4.5 minimum contrast correction, and uses transparent `#808080` as the conservative contrast reference at the −50% switch point. UI variables, terminal text halo, and title-bar symbols switch together. Policy-disabled and runtime-failure states always use an opaque `#181818` fallback with light foregrounds. Windows light/dark color preference does not select the palette; the user's contrast value does.
